 ...
 * 3. userId = 자신이 로그인한 아이디
 * 4. 미들웨어에서 받아온 아이디가 실제로 데이터 베이스에 있는지 확인.
 * 5. 팀원 변경된 캐릭터는 false 에서 ture 값으로 변경
 */
...
router.post("/team", authMiddleware, async (req, res, next) => {
    try {
      ...
      if (!account || !account.characters || account.characters.length === 0) {
        return res.status(400).json({ message: "캐릭터를 보유하고 있지 않습니다." });
      }
