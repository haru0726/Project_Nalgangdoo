...
router.post("/sell", authMiddleware, async (req, res) => {
    ...
    if (!userId) {
        return res.status(401).json({ message: "사용자 인증 실패. 로그인 해주세요." });
    }
    ...
    try {
        ...
        if (!characterListEntry || characterListEntry.quantity < quantity) {
            return res.status(400).json({ message: "보유한 캐릭터 수량이 부족합니다." });
        }
        ...
        return res.status(200).json({ message: "캐릭터 판매 완료.", data: { characterName, quantity } });
    } catch (err) {
        console.error(err);
